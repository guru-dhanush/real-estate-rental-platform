  ...
  return (
    <div className="relative h-[450px] w-full  rounded-2xl">
      {images.map((image, index) => (
        <div
          key={image}
          className={`absolute inset-0 rounded-xl transition-opacity m-4 duration-500 ease-in-out ${
        ...
        >
          <Image
            ...
            fill
            priority={index == 0}
            className="object-cover cursor-pointer rounded-xl transition-transform duration-500 ease-in-out"
          />
        </div>
      ))}
      <button
