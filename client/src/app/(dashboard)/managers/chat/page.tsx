...
const ManagerChatListPage = () => {
  ...
  const filteredChats = chats?.filter((chat: Chat) => {
    ...
    return (
      chat.tenant?.name?.toLowerCase().includes(search) ||
      chat.property?.name?.toLowerCase().includes(search) ||
      chat.messages?.some((msg: Message) => msg.content.toLowerCase().includes(search))
    );
  });
        ...
        <div className="flex flex-col items-center justify-center flex-1 p-6">
          ...
          <p className="text-gray-500 text-center max-w-md mb-6">
            {searchQuery
              ...
              : "When tenants contact you about your properties, conversations will appear here."}
          </p>
        </div>
      )}
